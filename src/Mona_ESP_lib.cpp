...
//Right Motor
void Mona_ESP::Right_mot_forward(int speed){
	if(speed>255){
		speed = 255; //Limit max speed to the 8 bit resolution
	}
	ledcWrite(Mot_rig_for_pwm, speed);
...
void Mona_ESP::Right_mot_backward(int speed){
	if(speed>255){
		speed = 255; //Limit max speed to the 8 bit resolution
	}
	ledcWrite(Mot_rig_for_pwm, 0);
...
void Mona_ESP::Right_mot_stop(void){
	ledcWrite(Mot_rig_for_pwm, 0);
	ledcWrite(Mot_rig_bac_pwm, 0);
}
//Left Motor
void Mona_ESP::Left_mot_forward(int speed){
	if(speed>255){
		speed = 255; //Limit max speed to the 8 bit resolution
	}
	ledcWrite(Mot_lef_for_pwm, speed);
...
void Mona_ESP::Left_mot_backward(int speed){
	if(speed>255){
		speed = 255; //Limit max speed to the 8 bit resolution
	}
	ledcWrite(Mot_lef_for_pwm, 0);
...
void Mona_ESP::Left_mot_stop(void){
	ledcWrite(Mot_lef_for_pwm, 0);
	ledcWrite(Mot_lef_bac_pwm, 0);
}

//Both motors
void Mona_ESP::Motors_forward(int speed){
	Right_mot_forward(speed);
	Left_mot_forward(speed);
}

void Mona_ESP::Motors_backward(int speed){
	Right_mot_backward(speed);
	Left_mot_backward(speed);
}

void Mona_ESP::Motors_spin_left(int speed){
	Right_mot_forward(speed);
	Left_mot_backward(speed);
}

void Mona_ESP::Motors_spin_right(int speed){
	Right_mot_backward(speed);
	Left_mot_forward(speed);
}

void Mona_ESP::Motors_stop(void){
	Right_mot_stop();
	Left_mot_stop();
}

//IR sensors
// void Mona_ESP::Enable_IR(void){
// 	digitalWrite(IR_enable,HIGH);
// }
//
// void Mona_ESP::Disable_IR(void){
// 	digitalWrite(IR_enable,LOW);
// }
//
// int Mona_ESP::Read_IR(int IR_number){
// 	if (IR_number>= 1 && IR_number<6){ //Ensure the IR number is within range
// 		if(IR_number==1){
...
// void Mona_ESP::LED2_ON(void){
// 	digitalWrite(LED_D2,HIGH);
// }
//
// void Mona_ESP::LED2_OFF(void){
// 	digitalWrite(LED_D2,LOW);
// }
//
// void Mona_ESP::LED3_ON(void){
// 	digitalWrite(LED_D3,HIGH);
// }
//
// void Mona_ESP::LED3_OFF(void){
// 	digitalWrite(LED_D3,LOW);
// }
